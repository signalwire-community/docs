...
This object is returned by [`sendDigits`](relay/server-sdk/php/reference/relay_calling/call.mdx#senddigits) method and represents the final result of a _send digits_ action.
...
```php title="Send some digits and then grab the last event occurred."
<?php
...
```php title="Send some digits and then check if it has ended successfully."
<?php
