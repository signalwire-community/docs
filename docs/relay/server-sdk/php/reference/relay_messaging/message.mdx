...
An object representing an SMS or MMS message. It is the parameter of both [`onIncomingMessage`](relay/server-sdk/php/reference/relay_consumer.mdx#onincomingmessage) and [`onMessageStateChange`](relay/server-sdk/php/reference/relay_consumer.mdx#onmessagestatechange) Consumer handlers.
...
|     `state` | <span class="method-prop-type">string</span>   | The current state of the message. See [`Relay.Messaging.Message` State Events](relay/server-sdk/php/reference/relay_messaging/message.mdx#state-events) for all the possible states. |
